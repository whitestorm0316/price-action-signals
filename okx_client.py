...
# 5m 约 1440 根、1m 约 1440 根），请求超过可得历史时提前返回不足一页即止。
...
class OKXClient:
    ...
    def get_candles(
        ...
        最多 MAX_TOTAL(3000) 根。实际返回可能少于请求值：
        OKX 最近行情端点只保留有限历史（5m 约 1440 根），
        更久远数据需走 history-candles 端点（当前未接）。

        Args:
            inst_id: 交易对，如 "BTC-USDT-SWAP"
            bar: K 线周期，如 "5m"（支持 1m/3m/5m/15m/30m/1H/4H/1D 等）
            limit: 返回条数，>300 自动分页，上限 MAX_TOTAL(3000)
        ...
        while len(rows) < total:
            page_n = min(total - len(rows), PAGE_SIZE)
            params = {
                "instId": inst_id,
                "bar": bar,
                "limit": str(page_n),
            ...
            # 本页最旧一根的 ts 作为下一页游标；返回不足一页说明已到头
            after = int(data[-1][0])
            if len(data) < page_n:
                break
            if len(rows) < total:
    ...
    def _request_with_retry(self, params: dict) -> list[list]:
